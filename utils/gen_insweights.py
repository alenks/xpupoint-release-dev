...
def get_args():
  parser = argparse.ArgumentParser(description="Generate instruction weights for clusters")
  parser.add_argument("-d", "--data-dir", type=str, 
                     help="SimPoint output directory (require T.global.hv, t.simpoints, t.labels)")
  parser.add_argument("-g", "--global-bbv", type=str, default="T.global.hv", 
...
def write_weights(datadir, cluster_weight, weight_sum):
  ...
  if abs(weight_sum - 1.0) > 0.01:
    logging.warning(f"Weight sum ({weight_sum}) deviates significantly from 1.0")
...
def main(datadir, globalbbv='T.global.hv'):
  ...
  try:
    ...
    cluster_weight, weight_sum = calc_cluster_weights(
      slice_weights, region_slice_map, region_counts
    )
