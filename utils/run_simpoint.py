...
def get_args():
  ...
  parser.add_argument("-m", "--maxk", type=int, default=20, 
                     help="maxK for Kmeans clustering (20)")
  parser.add_argument("-d", "--dim", type=int, default=15, 
                     help="Number of reduced dimensions (15)")
  parser.add_argument("-b", "--bbvfile", type=str, default="./T.global.hv", 
                     help="The BBV file to sample (T.global.hv)")
  parser.add_argument("-o", "--outdir", type=str, 
                     help="Output directory")
  parser.add_argument("--no-regions", default=False, action='store_true', 
                     help="No xpuregions.csv file will be generated")
  parser.add_argument("--fixed-length", type=str, default="on", 
                     help="Passed to SimPoint -fixedLength (on)/off")
  parser.add_argument("--gpu-only", default=False, action='store_true',
...
def run_simpoint(simpoint_bin, globalbbv, maxk, dim, outdir, fixed_length):
  ...
  cmd = [
    simpoint_bin,
    '-loadFVFile', globalbbv,
    '-maxK', str(maxk),
    '-dim', str(dim), 
    '-coveragePct', '1.0',
    '-saveSimpoints', tsimpoints,
    '-saveSimpointWeights', tweights,
    '-saveLabels', tlabels,
  ...
  try:
    ...
    raise RuntimeError(f'SimPoint failed with exit code {e.returncode}: {e.stderr}')
...
def gen_regions(globalbbv, tsimpoints, tweights, tlabels, outdir, gpu_only):
  ...
  cmd = [
    'python3', xpu_regions_script,
    f'--bbv_file={globalbbv}',
    f'--region_file={tsimpoints}', 
    f'--weight_file={tweights}',
    f'--label_file={tlabels}',
    '--csv_region'
  ]
  ...
  try:
    with open(regions_csv, 'w') as outfile:
      result = subprocess.run(cmd, check=True, stdout=outfile, stderr=subprocess.PIPE, text=True)
      if result.stderr:
        logging.warning(f'xpu_regions.py stderr: {result.stderr}')
  except subprocess.CalledProcessError as e:
    raise RuntimeError(f'Region generation failed with exit code {e.returncode}: {e.stderr}')
...
def main(maxk, dim, globalbbv, outdir, no_regions=False, gpu_only=False, fixed_length="on"):
  ...
  try:
    sde_kit, simpoint_bin = check_env()
    
    Path(outdir).mkdir(parents=True, exist_ok=True)
    
    tsimpoints, tweights, tlabels = run_simpoint(
      simpoint_bin, globalbbv, maxk, dim, outdir, fixed_length
    )
...
def run_cluster(maxk=20, dim=15, bbvfile="./T.global.hv", outdir=None, 
                  no_regions=False, gpu_only=False, fixed_length="on", verbose=False):
  if verbose:
...
if __name__ == '__main__':
  ...
  try:
    ...
    main(maxk, dim, globalbbv, outdir, args.no_regions, args.gpu_only, args.fixed_length)
